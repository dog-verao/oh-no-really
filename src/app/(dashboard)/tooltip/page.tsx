...
export default function AnnouncementsPage() {
  ...
  const {
    ...
    isDeleting,
    publishAnnouncement,
  } = useAnnouncements(account?.id || '', 'tooltip');

  const isLoading = isLoadingAccount || isLoadingAnnouncements;

  const handleRowClick = (params: GridRowParams) => {
    router.push(`/tooltip/${params.row.id}`);
  };

  const handleCreateNew = () => {
    router.push('/tooltip/new');
  };
  ...
  const handleEditAnnouncement = (announcementId: string) => {
    router.push(`/tooltip/${announcementId}/edit`);
  };
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 4 }}>
        <Box>
          ...
          <Typography variant="body1" color="text.secondary">
            Manage your tooltip announcements.
          </Typography>
        </Box>
        <Button
          variant="contained"
        ...
        {announcements.length === 0 ? (
          ...
          >
            <Typography variant="h6" color="text.secondary">
              No tooltip announcements found
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Create your first tooltip announcement to get started.
            </Typography>
            <Button
              variant="contained"
